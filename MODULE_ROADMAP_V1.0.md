...
| 7 | Video Automation | Content Engine | **FUNCTIONALLY COMPLETE / FROZEN** | Completed — checkpoint confirmed Module 7 extends Module 6's Content Scoring Engine (Video/Thumbnail dimensions registered alongside Blog, one shared composite, no fork — verified at freeze). Phases 7.1–7.8 shipped via PRs #59–72, all merged with green CI, final freeze SHA `78f562c`. Full brief→script→scene→assets→voice→subtitles→render→QA→SEO→review pipeline; dedicated `apps/render-worker` + real Remotion engine; real staging Remotion render + six-check QA both naturally PASS (Gates #4/#5 derived live, never hand-set); PG credential rotated; presigned media preview fixed to a real browser-reachable public endpoint (audio/image/video previews all live-verified from outside the VPS); login brute-force lock now cleared on a successful password reset; password policy widened to 8–64 characters; a genuine e2e test-cleanup race found and fixed during freeze regression (`video-render-qa.e2e-spec.ts`). Full regression green: 589 E2E tests (185 worker + 9 render-worker + 395 API) + Remotion smoke, on the final tagged SHA. VIDEO_RENDER/VIDEO_APPROVE RBAC and the generic-review-bypass seal both explicitly e2e-covered. Manual browser UAT (390px, 768px, keyboard navigation, video detail/pipeline usability) user-confirmed. Live AI provider golden path remains `BLOCKED_PROVIDER_CONFIGURATION` (no OpenAI/Azure credentials on staging) — non-blocking, same precedent as Module 6. Freeze tag: `v7.0.0-module-7-functionally-complete`. |
| 8 | AI Internal Linking Engine | Content Engine (cross-cutting) | **FUNCTIONALLY COMPLETE / FROZEN** | Completed — Phases 8.1–8.8 shipped via PRs #73–79 (8.1 domain/persistence, 8.2 candidate discovery + relevance, 8.3 deterministic anchor recommendation, 8.4 human-review API + Module 6 Blog seam, 8.5 orphan/cluster/link-health intelligence, 8.6 frontend, 8.7 staging deployment + UAT with zero defects found, 8.8 hardening + freeze), all merged with green CI. Freeze tag: `v8.0.0-module-8-functionally-complete`. Scope explicitly v1, Blog → Blog only (source DRAFT/IN_PROGRESS, target APPROVED only); deterministic throughout — zero AI/provider dependency for any required capability, proven end to end on real staging infrastructure with no provider credentials configured. Frozen product decision: a REJECTED source/target pair may be re-suggested by a later Generate run (REJECTED is history, not a permanent suppression) — intentional per the Phase 8.1 partial-unique-index design, not a defect; the frontend now surfaces a small contextual hint when this applies. Non-blocking carried debt (§10 freeze record has the full narrative): the `reconcileWorkspace()` scheduler wrapper (read-time safety alone fully covers correctness, staging UAT proved this), `internalLinkingPolicy.maxLinksPerArticle` (parsed but not behaviorally wired to generation limits — never surfaced to users as an enforced promise, so no correctness/security exposure), and anchor-edit audit granularity (captured in the row's own evidence JSON, not a dedicated AuditAction — no frozen security requirement mandates one). |
| 9 | Publishing Engine | Distribution | **FUNCTIONALLY COMPLETE / FROZEN** | Completed — Phases 9.1–9.7 shipped via PRs #80–86 (9.1 persistence foundation, 9.2 provider/readiness abstraction, 9.3 scheduler/worker dispatch, 9.4 WordPress connector, 9.5 YouTube connector + a pre-merge checkpoint-encryption security correction, 9.6 Facebook/Instagram connectors, 9.7 account management + publication UI), all merged with green CI. Phase 9.8 staging deployment + UAT (backup `pre_module9_20260905T074320Z.sql.gz`, migration `20260904083027_module_9_phase_9_1_publishing_foundation` applied, zero pending) found and fixed one real defect: the staging deploy runbook's `prisma migrate deploy` step never re-runs the idempotent RBAC seed (`seedRbac()`), so `PUBLISH_CHANNEL_MANAGE` — added by this module — was silently absent from every pre-existing workspace's Owner/Administrator role until the seed was run manually; fixed on staging and the deploy runbook updated to require it on every future deploy that adds a permission constant. RBAC (read vs. manage split), workspace isolation, OAuth graceful-degradation-when-unconfigured (clean `422 PUBLISHING_OAUTH_PROVIDER_NOT_CONFIGURED`, no crash), and the WordPress validate-before-persist path were all proven live against real staging infrastructure. Real end-to-end provider golden paths (WordPress publish, YouTube/Meta OAuth + publish) remain `BLOCKED_EXTERNAL_CONFIGURATION` — no OAuth client credentials configured and no authorized safe WordPress target on staging, same precedent as Module 6/7's `BLOCKED_PROVIDER_CONFIGURATION` — not a code defect. Facebook's disclosed crash-after-non-idempotent-publish and Instagram's narrow unrecoverable-media-id race (Phase 9.6) are operator-actionable via the Phase 9.7 manual-reconciliation UI rather than silently stuck, verified via deterministic tests. Deterministic test suite green throughout (shared/worker/api/web). Freeze tag: `v9.0.0-module-9-functionally-complete`. **Post-freeze stabilization** (main advanced to `7bd4d92` after the freeze tag, which remains untouched at `c17f92e`): PR #88 added a real authenticated `SmtpEmailProvider` (TLS, port 465) alongside the existing `MailpitEmailProvider` — Mailpit stays the local/test default; staging now sends through a real mailbox (`noreply@evspine.com` via Hostinger, smoke-verified) instead of being permanently Mailpit-only. PR #89 fixed a real defect manual browser UAT found: the publish wizard's "Select content" step reused `BlogService.list()`/`VideoService.list()`, which are deliberately scoped to items carrying Module 6/7 pipeline metadata — a real Approved Blog created outside that pipeline (e.g. a Module 8 UAT fixture) was silently invisible to Publishing. Fixed via a dedicated `GET .../publications/content-candidates` querying `ContentItemsService.list()` directly; zero Module 6/7/8 files touched. Browser UAT re-confirmed the fix: the Approved Blog now appears, is selectable, and the channel-compatibility gate at Step 2 behaves correctly. Real external-provider golden-path UAT (WordPress/YouTube/Facebook/Instagram) remains `BLOCKED_EXTERNAL_CONFIGURATION`, deferred until dedicated test accounts/destinations exist — not a defect. Patch tag: `v9.0.1-defect-9-001`. |
...
**Module 8 — AI Internal Linking Engine**
...
- Major capabilities: knowledge-graph-driven internal linking (deterministic candidate discovery, relevance scoring, anchor recommendation, human review), orphan-content detection, topic-cluster link-health intelligence
- **Status: FUNCTIONALLY COMPLETE / FROZEN** (see §5's table row and §10's freeze tag for the full record). Delivered exactly `AI_INTERNAL_LINKING_ENGINE_V1.0.md`'s scope, Blog → Blog v1. General on-page metadata/schema optimization and standalone ranking-probability recommendations (distinct capabilities the original roadmap line grouped under the same "SEO Engine" label but which were never covered by a cited FRD/spec section beyond this internal-linking doc) were never built and remain open — a future module or Module 8 v2 extension, not a Module 8 v1 defect or partial delivery.
...
- **Status: FUNCTIONALLY COMPLETE / FROZEN** (see §5's table row for the full record). Delivered the shared publishing core across WordPress, YouTube, Facebook, and Instagram — connectors, scheduling, retry/cancel, and account management/UI — plus a manual-reconciliation domain for the two channels' disclosed non-idempotent-outcome edge cases. Real external-provider golden paths are `BLOCKED_EXTERNAL_CONFIGURATION` on staging (no OAuth app credentials, no authorized WordPress target), not a defect.
...
| 7 | FUNCTIONALLY COMPLETE / FROZEN | Modules 5, 6 ✓ |
| 8 | FUNCTIONALLY COMPLETE / FROZEN | Modules 6 ✓, 7 ✓ |
| 9 | FUNCTIONALLY COMPLETE / FROZEN | Module 8 ✓ |
